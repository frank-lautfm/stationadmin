...
public class ScheduleEditor extends JPanel {
  ...
  public ScheduleEditor(ClientContext ctx) {
    ...
    this.model = new ScheduleTableModel(ctx.getTextProvider(), ctx.getAdminClient().getPlaylistService().getPlaylistRegistry(), ctx.getAdminClient().getSchedule());
    this.init();
  }
      ...
      try {
        for (DataFlavor flavor : support.getDataFlavors()) {
          if (flavor.equals(DataFlavor.stringFlavor)) {
            ...
            for (Playlist playlist : ctx.getAdminClient().getPlaylistService().getPlaylistRegistry().getAllPlaylists()) {

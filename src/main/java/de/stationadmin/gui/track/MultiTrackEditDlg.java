...
import de.stationadmin.gui.upload.TrackTypeListCellRenderer;
...
class MultiTrackEditDlg extends JDialog {
  ...
  public MultiTrackEditDlg(ClientContext ctx, List<DetailedTrack> tracks) {
    super();
    this.ctx = ctx;
    this.tracks = tracks;
    this.init();
  ...
  private void apply() {
    for (DetailedTrack track : tracks) {
      ...
      if (this.artistEnabledCb.isSelected()) {
        track.setArtist(this.artistTf.getText());
      }
      if (this.titleEnabledCb.isSelected()) {
        track.setTitle(this.titleTf.getText());
      }
      if (this.albumEnabledCb.isSelected()) {
        track.setAlbum(this.albumTf.getText());
      }
      if (this.genreEnabledCb.isSelected()) {
        track.setGenre(this.genreTf.getText());
      }
      ...
      if (this.typeEnabledCb.isSelected()) {
        track.setType((Integer) typeCmb.getSelectedItem());
      }
      if (this.privateEnabledCb.isSelected()) {
        track.setPrivateTrack(this.privateCb.isSelected());
      }

      try {
        ctx.getAdminClient().getTrackService().updateTrack(track);
      } catch (Exception e) {
        JXErrorPane.showDialog(this, ctx.createErrorInfo(e, "action.multitrackedit.msg.failed", track.getArtist() + " - " + track.getTitle()));
        try {
          ctx.getAdminClient().getTrackService().reloadTrack(track.getId());
        } catch (Exception e2) {
        }
      }

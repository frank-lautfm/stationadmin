...
class PlaylistEditPropertiesAction extends AbstractAction implements PropertyChangeListener {
  ...
  public void actionPerformed(ActionEvent e) {
    ...
    if (playlist != null) {
      PlaylistConfigurationModel model = new PlaylistConfigurationModel(playlist, this.ctx.getAdminClient().getPlaylistService(), this.ctx.getAdminClient().getTagManager(), ctx.getAdminClient().getPlaylistService().getShuffleScripts(), ctx.getAdminClient().getPlaylistService().getProfiles(), ctx.getTextProvider());

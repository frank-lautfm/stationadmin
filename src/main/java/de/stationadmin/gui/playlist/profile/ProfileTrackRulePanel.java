...
public class ProfileTrackRulePanel extends JPanel {
  private static final long serialVersionUID = 7997929549512395824L;
  private ClientContext ctx;
  private TrackRuleModel model;

  public ProfileTrackRulePanel(ClientContext ctx, PlaylistProfileModel profileModel) {
    this.ctx = ctx;
    this.model = profileModel.getTrackRuleModel();
    this.init();
  }
  ...
  private void init() {
    ...
    this.setLayout(new FormLayout("5dlu,100dlu:grow,5dlu", "5dlu,pref,5dlu,30dlu:grow,8dlu,pref,5dlu,50dlu:grow,8dlu,pref,3dlu,pref,8dlu,pref,3dlu,pref,5dlu,pref"));
    CellConstraints cc = new CellConstraints();

    this.add(new JLabel("Gruppen"), cc.xy(2, 2));

    final TrackRuleGroupTableModel groupsModel = new TrackRuleGroupTableModel(ctx.getTextProvider());
    JXTable groupsTable = new JXTable(groupsModel);
    this.add(new JScrollPane(groupsTable), cc.xy(2, 4));
    {
    ...
    groupsModel.addNameChangeListener(new PropertyChangeListener() {
      ...
      public void propertyChange(PropertyChangeEvent evt) {
        ...
        if (StringUtils.isNotEmpty(old)) {
          for (TrackRule rule : model.getRules()) {
    ...
    this.add(new JLabel("Regeln"), cc.xy(2, 6));

    final TrackRuleTableModel rulesModel = new TrackRuleTableModel(ctx.getTextProvider(), ctx.getAdminClient().getTrackService().getTrackRegistry());
    final JXTable rulesTable = new JXTable(rulesModel);
    this.add(new JScrollPane(rulesTable), cc.xy(2, 8));

    final IndirectListModel<TrackRuleGroup> groupsListModel = new IndirectListModel<>();
    {
      TableColumn colGroup = rulesTable.getColumnModel().getColumn(0);
      SelectionInList<TrackRuleGroup> groupSelection = new SelectionInList<TrackRuleGroup>(groupsListModel);
    ...
    SelectionInList<MultiMatchSelection> groupCollisionStrategySelection = new SelectionInList<TrackRuleGroup.MultiMatchSelection>(
        new MultiMatchSelection[] { MultiMatchSelection.ALL, MultiMatchSelection.FIRST, MultiMatchSelection.RANDOM }, model.getBufferedComponentModel("groupCollisionStrategy"));
    ...
    this.add(new JLabel(ctx.getString("settings.playlistgen.table.rule.group.collission")), cc.xy(2, 10));
    this.add(groupCollisionStrategyCmb, cc.xy(2, 12, CellConstraints.LEFT, CellConstraints.CENTER));
    ...
    SelectionInList<JingleCollisionStratagy> jcsSelection = new SelectionInList<JingleCollisionStratagy>(JingleCollisionStratagy.values(),
        model.getBufferedComponentModel("jingleCollisionStrategy"));
    ...
    this.add(new JLabel(ctx.getString("settings.playlistgen.table.rule.collission")), cc.xy(2, 14));
    this.add(jcsCmb, cc.xy(2, 16, CellConstraints.LEFT, CellConstraints.CENTER));
    ...
    model.getBufferedModel("rules").addValueChangeListener(new PropertyChangeListener() {

      @Override
      public void propertyChange(PropertyChangeEvent evt) {
        rulesModel.setRules(model.getRules());
      }
    });

    model.getBufferedModel("groups").addValueChangeListener(new PropertyChangeListener() {

      @Override
      public void propertyChange(PropertyChangeEvent evt) {
        rulesModel.setGroups(model.getGroups());
        groupsModel.setGroups(model.getGroups());
        groupsListModel.setList(model.getGroups());
      }
    });

...
public class ScheduledItemPanel extends JPanel {
	...
	private void initRules() {
		...
		try {
			rules.sort((r1, r2) -> r1.getScheduledItem().getName().compareToIgnoreCase(r2.getScheduledItem().getName()));
		} catch (Exception e) {
		}
		for (ScheduledItemRule rule : rules) {
	...
	private JPanel createScheduledItemPanel() {
		...
		JPanel panel = new JPanel(
				new FormLayout("pref,5dlu,max(pref;120dlu)", "pref,5dlu,pref,5dlu,pref,5dlu,pref,5dlu,pref"));
		CellConstraints cc = new CellConstraints();
		int row = 1;

		{
			List<ScheduledItem> items = new ArrayList<>(ctx.getAdminClient().getPlaylistService().getScheduledItems());
			try {
				items.sort((i1, i2) -> i1.getName().compareToIgnoreCase(i2.getName()));
			} catch(Exception e) {
			}
			
			SelectionInList<ScheduledItem> itemSelectionInList = new SelectionInList<>(items, scheduledItem);
		...
		}
		{
			...
			JComboBox<Integer> daySelection = BasicComponentFactory.createComboBox(daySelectionInList,
		...
		}
		{
			...
			JComboBox<ScheduledItem> hourSelection = BasicComponentFactory.createComboBox(hourSelectionInList,
					new DefaultListCellRenderer() {
...

		{
			...
			final JComboBox<ScheduledItem> intervalSelection = BasicComponentFactory.createComboBox(intervalSelectionInList,
					new DefaultListCellRenderer() {
						private static final long serialVersionUID = -1236436512327071569L;
						...
						public Component getListCellRendererComponent(JList<?> list, Object value, int index, boolean isSelected,
							...
							try {
								...
								if (iVal == 0) {
									...
									setText(
											ctx.getString("playlistcfg.property.scheduleditem.interval.minutex", Integer.toString(-iVal)));
								} else {
	...
	private void init() {
		this.setLayout(new FormLayout("5dlu,min(pref;220dlu):grow,5dlu",
				"5dlu,pref,8dlu,100dlu,15dlu,pref,5dlu,pref,15dlu,pref,15dlu,pref,5dlu"));
		CellConstraints cc = new CellConstraints();
		int row = 2;
		...
		{
			SelectionInList<ScheduledItemRule> ruleSelection = new SelectionInList<ScheduledItemRule>(ruleListModel,
					this.selection);
	...
	private class AcceptAction extends AbstractAction {
		...
		public void actionPerformed(ActionEvent e) {
			...
			ScheduledItemRule rule = selection.getValue() != null ? (ScheduledItemRule) selection.getValue()
					: new ScheduledItemRule();

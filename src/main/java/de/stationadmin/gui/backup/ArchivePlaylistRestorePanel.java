 ...
 * 
 */
public class ArchivePlaylistRestorePanel extends JPanel {
  ...
  private Map<String, JCheckBox> playlistCbs = new HashMap<String, JCheckBox>();
  ...
  /**
     ...
     * @param adminClient
     */
  public ArchivePlaylistRestorePanel(TextProvider textProvider, StationAdminClient adminClient, File file) {
    super();
    this.file = file;
  ...
  private void init() {
    ...
    BackupService tool = this.adminClient.getBackupService();
    try {
      List<String> files = tool.getArchivePlaylists(file);

      {
        ...
        for (String file : files) {
          ...
          cb.putClientProperty("playlist", file);
          playlistPanel.add(cb);
          this.playlistCbs.put(file, cb);
        }
  ...
  private class ImportAction extends ThreadedAction {
    ...
    protected void performAction() throws Exception {
      ...
      for (JCheckBox cb : playlistCbs.values()) {
        if (cb.isEnabled() && cb.isSelected()) {
          String filename = (String) cb.getClientProperty("playlist");
          this.status = textProvider.getString("backup.restore.playlist.import.status", filename);
          tool.restoreArchivePlaylist(file, filename);
          cnt++;
        }
      }
    ...
    protected void showError(Exception e) {
      JXErrorPane.showDialog(AppUtils.getRootFrame(),
          textProvider.createErrorInfo(e, "backup.restore.playlist.import.failed"));
    }

 ...
 * 
 */
public class TaskRestorePanel extends JPanel {
  private static final long serialVersionUID = -7386236089137928548L;
  // private ClientContext ctx;
  private BackupService backupService;
  private TextProvider textProvider;
  private File file;
  private List<JCheckBox> taskCbs = new ArrayList<JCheckBox>();

  public TaskRestorePanel(ClientContext ctx, File file) {
    super();
  ...
  private void init() {
...

    {
      ...
      head.add(new JLabel(textProvider.getString("backup.restore.task.head")));
...

    {
      ...
      try {
        List<ScheduledTask> tasks = backupService.getAvailableTasks(file);

        for (ScheduledTask task : tasks) {
          String label = task.getTask().getName() != null ? task.getTask().getName() : this.textProvider.getString("task.type." + task.getTask().getClass().getSimpleName());
          JCheckBox cb = new JCheckBox(label);
          cb.putClientProperty("task", task);
          taskPanel.add(cb);
      ...
      this.add(new JScrollPane(taskPanel), cc.xy(2, 4, CellConstraints.FILL, CellConstraints.TOP));
    }
  ...
  private class ImportAction extends ThreadedAction {
    ...
    protected void performAction() throws Exception {
      for (JCheckBox cb : taskCbs) {
        if (cb.isEnabled() && cb.isSelected()) {
          ScheduledTask task = (ScheduledTask) cb.getClientProperty("task");
          this.status = textProvider.getString("backup.restore.task.import.status");
          backupService.restoreTask(task);
          cnt++;
        }
      }
    ...
    protected void onSuccess() {
      if (cnt > 0) {
        JOptionPane.showMessageDialog(AppUtils.getRootFrame(), textProvider.getString("backup.restore.task.import.succeeded", Integer.toString(cnt)));
        for (JCheckBox cb : taskCbs) {
          if (cb.isEnabled() && cb.isSelected()) {

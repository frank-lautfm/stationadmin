...
public class DynamicTag implements Tag {
	...
	private class Matcher {
		...
		public boolean matches(BasicTrack title) {
			if (matches(this.artistPatterns, title.getArtist()) && matches(this.titlePatterns, title.getTitle())
					&& title.getLength() > minLength && title.getLength() < maxLength) {
				if (this.albumPatterns != null) {
					return (title instanceof RegisteredTrack)
							&& matches(this.albumPatterns, ((RegisteredTrack) title).getAlbum());
				} else {
					return true;
				}
			}

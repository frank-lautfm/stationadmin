...
public class LogAnalyzerService implements Service {
	...
	private String getRawDataOf(String type, Date day) throws IOException {
		...
		if (day.getTime() > System.currentTimeMillis() - (DAY_IN_MS * 100l)) {
			...
			for (TrackStatsEntry entry : stats) {
				...
				if ((entry.isLive() || entry.getId() > 0)
						&& calDay.get(Calendar.DAY_OF_MONTH) == calEntry.get(Calendar.DAY_OF_MONTH)
		...
		for (String line : lines) {
			...
			if (cols.length == 2) {
				try {
					...
					BasicTrack track = isLiveTrack ? this.trackRegistry.getLiveTrack(trackId)
							: this.trackRegistry.getTrack(trackId);
					if (track == null) {
	...
	public List<ListenersAvgEntry> getAverageListenersInDay(List<ListenersEntry> rawEntries, int weekdays,
			int granularity) {
		checkAccess();
	...
	public List<MonthlySummary> getMonthlySummary() throws IOException {
		...
		ListenerStatsEntry[] entries = ctx.getServer().getListenerStatistics(ctx.getStationId(), cal.getTime(), new Date(),
				ListenerStatsPeriod.Monthly);
		...
		this.ctx.getStationStatus().addPropertyChangeListener("currentListeners", new PropertyChangeListener() {
			...
			public void propertyChange(PropertyChangeEvent evt) {
				if (listenersToday != null) {
					listenersToday.add(
							new ListenersEntry(new Date(System.currentTimeMillis()), ctx.getStationStatus().getCurrentListeners()));
				}
			}
		});
	...
	private void writeDailyStatistics() throws IOException {
		...
		ListenerStatsEntry[] listenerStats = ctx.getServer().getListenerStatistics(ctx.getStationId(), cal.getTime(), today,
				ListenerStatsPeriod.Daily);
		...
		for (int i = 0; i < 31; i++) {
			...
			for (int j = 0; j < listenerStats.length; j++) {
				if (dateFmt.format(listenerStats[j].getDate()).equals(date)
						&& listenerStats[j].getSources().containsKey("all")) {
